...
class Account_holder(User):
    ...
    def withdraw_money(self,bank, amount):
        if bank.bankrupt == True:
            print("Loan is not possible due to bankrupt")
        else:
    ...
    def check_transaction_history(self,bank):
        bank.transaction_history_checking(self.name)
...
class Admin(User):
    ...
    def total_balance_of_bank(self, bank):
        print(f"Total Balance of the bank {bank.total_balance}")

    def total_loan_of_bank(self, bank):
        print(f"Total loan amount of the bank {bank.total_loan}")
